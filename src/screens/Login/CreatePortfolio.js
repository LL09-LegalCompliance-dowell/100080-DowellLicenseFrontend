...
const CreatePortfolio = ({navigation, route}) => {
  ...
  const getPortfolio = async () => {
    ...
    try{
      ...
      const {owner_name, member_type, org_name, portfolio_name, role} = portfolio[0];
      ...
      if(!portfolio.length){
        ...
        navigation.navigate("NoPortfolio", {"session_id":session_id, "username":username | owner_name})
        setLoading(false)
      }else{
  ...
  return (
    <>
      ...
      />
      <TouchableOpacity style={[styles.createPortfolio, {marginTop: 0,}]} onPress={fetchPortfolio}>
        <Text style={[styles.createPortfolioText, {fontSize:16}]}>{loading? "Checking Portfolio...":"Done Creating Portfolio?"}</Text>
      </TouchableOpacity>      
    </>
  );
};
